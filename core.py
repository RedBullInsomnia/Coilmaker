...
class Core(myThreads.Thread):
    def __init__(self,ser,memory,event,nouvelleBobineEvent,errorEvent,entrees, sorties,dataLogger):
        ...
        self.error = False
        #   Tant qu'on n'a pas chargé une bobine dans le système,
        #   file est vide. Les variables sont à zéro et les données bobines vides
        self.file = ''
    ...
    def run(self):    
        ...
        #   On attend d'avoir chargé une bobine
            ...
            #   Il faut attendre qu'on l'ait démarrée
                self.variables = DEFINE.VARIABLES.copy()#   On réinitialise les variables
    ...
    def makeRapport(self):
        comp = repport.PdfLatex(self.file,self.error,self.variables,self.mem.pos.couche)
        comp.start()
    ...
    def makeBobine(self):
        try:
            ...
            tStart = 0#     Stock le temps de début
            tcumul = 0#     Stock le temps cumulé (sans les pauses)
            tStop = 0#      Stock le temps passé en pause
            chronoTempsPause = 0
            ...
            # RPMprec = 0#    Stock le RPM    (consigne)
            # AVANCEprec = 0# Stock l'avance  (consigne)
            pauseAlreadyHappen = False# Est à True si la pause est passée
            ...
            tab = [2,0x01,0x00,0x00,0x00,0x00,int(int(RPMprec*(2047/120.0))/256),int(int(RPMprec*(2047/120.0))%256)]
            iF.addChecksum(tab)
            self.ser.write(tab)
            ...
            increment = 0
            #   Boucle principale
            while ((self.stop == False) and self.bobineDemarree):
                self.read()#        On consulte les données envoyées par l'interface
                if ((int(self.mem.CT.tour) >= int(NTOURS))and (increment == 0)):
                #   Condition d'arrêt
                    self.l.debug('On a fini la bobine')
                    self.ser.write([2,0x03,0x00,0x00,0x00,0x00,0x00,0x00,0x04])
                    self.dataLogger.event("La bobine s'est terminé avec succès")
                    # self.nouvelleBobine.clear()
                    self.mem.SH.setEnable(False)
                    increment = 1
                    # self.bobineDemarree = False
                variableSortie += increment
                #   VariableSortie permet d'attendre que tout s'arrête correctement
                ...
                #   On sort de pause
                    ...
                    tab = [2,0x01,0x00,0x00,0x00,0x00,int(float(saveRPM)*(2047/120.0)/256.0),int(float(saveRPM)*(2047/120.0)%256)]
                    iF.addChecksum(tab)
                    self.ser.write(tab)
                ...
                if (moyenneCourte == sizeMCourte):
                    if float(self.variables[DEFINE.RPM]) != float(RPMprec):
                        ...
                        iF.addChecksum(tab)
                        self.ser.write(tab)
                        tab = [1,0x05,0x04,0x00]+ iF.convertToOctet(int(AVANCEprec*RPMprec*(2047/60000.0)))
                        iF.addChecksum(tab)
                        self.ser.write(tab)
