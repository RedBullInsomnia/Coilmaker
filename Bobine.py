...
class wBobine(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        partDATA = tk.LabelFrame(self, text ="Projet", padx = 10, pady = 10, relief = tk.GROOVE)
        partL = tk.Frame(self)
        ...
        for i in DEFINE.BOBINE_order:
            self.champ_LabelDATA.append(tk.Label(partDATA, text=DEFINE.TRANSLATE_BOBINE[i] + " :"))
            self.champ_VarDATA.append(tk.Entry(partDATA,width=16, textvariable=(self.dicoBobine)[i]))
        

        for indice in self.vecteur09:
            (self.champ_LabelDATA)[indice].grid(row = indice, column = 0, sticky = "E")
            
        for indice in self.vecteur09:
            (self.champ_VarDATA[indice]).grid(row = indice, column = 1)

        self.IDbobine = tk.StringVar()
        self.IDbobine.set("ID bobine")
        (self.champ_VarDATA[4]).configure(textvariable = self.IDbobine,state = tk.DISABLED)
        self.boutongenerateID = tk.Button(partDATA, text = "Générer ID",anchor=tk.W,width= 16, command = self.updateID)
        self.boutongenerateID.grid(row = 4, column = 2)
        self.boutonChangeID = tk.Button(partDATA, text = "Modifier ID", anchor=tk.W, width= 16, command = self.modID)
        self.boutonChangeID.grid(row = 4, column = 3)

        partDATA.pack(side = tk.TOP)
        boutonOk = tk.Button(partL, text = "Valider",anchor=tk.W, command = self.validate)
        boutonOk.pack()
    ...
    def updateID(self):
        ...
        (self.IDbobine).set('{}_{}_{}_{}'.format(text[0][0:2],text[1][0:3],text[2],text[3]))
        
    def modID(self):
        (self.champ_VarDATA[4]).config(state = tk.NORMAL)
        (self.boutonChangeID).config(text = "Valider ID", command = self.validateID)


    def validateID(self):
        (self.champ_VarDATA[4]).config(state = tk.DISABLED)
        (self.boutonChangeID).config(text = "Modifier ID", command = self.modID)

        
class openWBobine(wBobine):
    """Défini une nouvelle fenêtre. Cette fenêtre va permettre d'ouvrir une nouvelle bobine"""
...
class newWBobine(wBobine):
    """Défini une nouvelle fenêtre. Cette fenêtre va permettre de créer une nouvelle bobine"""
    def __init__(self, parent):
        wBobine.__init__(self,parent)
        self.parent = parent
